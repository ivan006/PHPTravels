 ...
 // Room Price
		...
		function hotel_details() {
			   ...
			   //	$this->isfeatured = $this->is_featured();
                ...
                $detailResults = (object)array('id' => $details[0]->hotel_id, 'title' => $title, 'slug' => $slug,'bookingSlug' => $bookingSlug, 'thumbnail' => $thumbnail,
            'stars' => pt_create_stars($stars), 'starsCount' => $stars, 'location' => $city->city,'desc' => $desc,'amenities' => $amenities,
            'latitude' => $latitude,'longitude' => $longitude,'sliderImages' => $sliderImages,'relatedItems' => $relatedHotels,'paymentOptions' => $paymentOptions,
...
// List 2 top rated hotels from each city passed to to this function
		...
		public function suggestionResults($query){
        	...
        	if(!empty($res)){
        		...
        		$hotels[] =  (object)array('id' => '', 'name' => '', 'module' => 'hotel', 'disabled' => true);
        	foreach($res as $r){
        		$title = $this->get_title($r->title,$r->id);
        		//$hotels[] =  (object)array('id' => array('dataType' => 'hotels', 'id' => $r->id), 'name' => trim($title), 'module' => 'hotels');  
        		$hotels[] =  (object)array('id' => $r->id, 'name' => trim($title), 'module' => 'hotel', 'disabled' => false);  
        	}
